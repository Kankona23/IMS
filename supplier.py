...
class supplierClass:
    def __init__(self,root):
        ...
        #===========row4=========
        lbl_decs=Label(self.root,text="Description",font=("goudy old style",15),bg="white").place(x=50,y=200)
...
#==================================================== 
    ...
    def update(self):
        ...
        try:
            if self.var_sup_invoice.get()=="":
                messagebox.showerror("Error","Supplier Must be required",parent=self.root)
            else:
                cur.execute("Select * from supplier where invoice=?",(self.var_sup_invoice.get(),)) 
                row=cur.fetchone()
                if row==None:
